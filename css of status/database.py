...
def upload_to_roboflow(image_path, object_name):
    url = "https://api.roboflow.com/dataset/eyeknow/upload"
    api_key = "DirYUZK6gfkQpBLKrJgv"  # Private API key
    ...
    with open(image_path, "rb") as image_file:
        response = requests.post(
            ...
            params={
                "api_key": api_key,
                "name": object_name,
                "split": "train"  # or "valid", "test" depending on your workflow
            },
            files={
                "file": image_file
            }
    ...
    if response.status_code == 200:
        print(f"✅ Uploaded to Roboflow: {image_path}")
        return True
    else:
        print(f"❌ Failed to upload to Roboflow: {image_path} | Status Code: {response.status_code}")
        print(response.text)
        return False
    ...
    try:
        ...
        try:
            ...
            for file, file_size in valid_files:
                ...
                upload_success = upload_to_roboflow(file_path, object_name)
