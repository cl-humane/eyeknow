...
ROBOFLOW_API_KEY = "DirYUZK6gfkQpBLKrJgv"
ROBOFLOW_WORKSPACE = "eyeknow"
ROBOFLOW_PROJECT = "eyeknow"
...
def upload_object_as_batch_to_roboflow(object_dir, object_name, object_id):
    ...
    Each object becomes one batch in the unassigned section
    """
    try:
            ...
            for root, dirs, files in os.walk(object_dir):
                for file in files:
                    if file.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.bmp', '.webp', '.svg')):
                        ...
                        # Use original file name in zip to maintain readability
                        arcname = file
        ...
        # Upload batch to Roboflow using the project.upload method
        try:
            print(f"🚀 Uploading batch '{batch_name}' to Roboflow...")
            
            # Upload the batch - this will go to "unassigned" by default
            upload_result = project.upload(
                image_path=zip_path,
                num_retry_uploads=3,
                batch_name=batch_name,  # This ensures it's treated as a single batch
                split="train"  # You can change this to "valid" or "test" if needed
            )
...
def upload_individual_images_to_roboflow_api(object_dir, object_name, object_id):
    ...
    This method uploads each image separately but tags them with the same batch identifier
    """
        ...
        try:
            with open(file_path, "rb") as image_file:
                ...
                params = {
                    ...
                    "split": "train",
                    "batch": batch_id  # This groups images into the same batch
                }
...
def get_current_model():
    conn = sqlite3.connect('admin.db')
    c = conn.cursor()
    c.execute('SELECT model_name, version_number, api_link, uploaded_at FROM model_version ORDER BY uploaded_at DESC LIMIT 1')
    model = c.fetchone()
    conn.close()
    ...
    if model:
        ...
        return jsonify({'message': 'No model uploaded yet'})
...
def edit_object():
    ...
    try:
        ...
        try:
            ...
            saved_files = []
            if valid_files:
                # Use the NEW object name for directory
